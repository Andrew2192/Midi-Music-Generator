...
from music_transformer.vocabulary import *
from music_transformer.vocabulary import events_to_indices
from music_transformer.masking import create_mask
...
def list_parser(index_list, fname=None, tempo=120):
    from music_transformer.vocabulary import indices_to_events
    import pretty_midi

    events = indices_to_events(index_list)
    ...
    for event in events:
        if event.startswith("note_on_"):
            ...
            note_on_time[pitch] = start_time  # store when the note starts
        elif event.startswith("note_off_"):
            pitch = int(event.split("_")[-1])
            if pitch in note_on_time:
                ...
                del note_on_time[pitch]
        elif event.startswith("time_shift_"):
            shift = int(event.split("_")[-1])
            start_time += shift / 100.0  # convert to seconds, adjust scaling as needed
